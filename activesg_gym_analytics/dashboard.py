...
function renderRank(rows){ const metric=$('metricSelect').value, by=new Map(); for(const r of rows){ const v=scoreValue(r,metric); if(v===null) continue; if(!by.has(r.gym_name)) by.set(r.gym_name,[]); by.get(r.gym_name).push(v); } const ranked=[...by.entries()].map(([g,vals])=>[g,avg(vals)]).sort((a,b)=>b[1]-a[1]).slice(0,12).reverse(); Plotly.newPlot('rankChart',[{x:ranked.map(x=>x[1]),y:ranked.map(x=>x[0]),type:'bar',orientation:'h',marker:{color:'#1667d9'}}],{margin:{t:8,r:20,l:10,b:35},yaxis:{automargin:true,tickfont:{size:10}},xaxis:{range:[0,100],title:'Avg score'}},{responsive:true,displayModeBar:false}); }
...
function renderHeatmap(rows){ const metric=$('metricSelect').value, gyms=[...new Set(rows.map(r=>r.gym_name))].sort(), hours=[...new Set(rows.map(hourOf))].sort(); const matrix=gyms.map(g=>hours.map(h=>avg(rows.filter(r=>r.gym_name===g && hourOf(r)===h).map(r=>scoreValue(r,metric)).filter(v=>v!==null)))); const chart=$('heatmapChart'), height=Math.max(420,gyms.length*22+120); chart.style.height=height+'px'; Plotly.newPlot(chart,[{z:matrix,x:hours,y:gyms,type:'heatmap',colorscale:[[0,'#e8f7ee'],[.45,'#fff4df'],[.75,'#ffb86b'],[1,'#d92d20']],zmin:0,zmax:100,colorbar:{title:'Score'}}],{margin:{t:10,r:20,l:10,b:45},yaxis:{automargin:true,tickfont:{size:10}},xaxis:{title:'Hour'},height},{responsive:true,displayModeBar:false}); }
