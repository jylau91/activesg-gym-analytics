...
function renderLatest(){ const latestTs=DATA.meta.last_fetched_at_sgt, latest=DATA.observations.filter(r=>r.fetched_at_sgt===latestTs).sort((a,b)=>a.gym_name.localeCompare(b.gym_name)); $('latestGrid').innerHTML=latest.map(r=>{ const s=r.crowd_score??r.occupancy_pct; const klass=(r.is_open===false)?'closed':s>=75?'bad':s>=45?'warn':'good'; const label=r.is_open===false?'Closed':(s==null?'Open / unknown':Math.round(s)+' score'); return `<div class="gym-card"><div class="gym-name">${escapeHtml(r.gym_name)}</div><div class="status">${escapeHtml(r.status_text)}</div><span class="pill ${klass}">${label}</span></div>`}).join(''); }
...
def export_data(db_path: Path = DB_PATH, site_dir: Path = SITE_DIR) -> dict:
    ...
    with connect(db_path) as conn:
        rows = conn.execute("""
            SELECT fetched_at_utc, fetched_at_sgt, gym_name, status_text, is_open,
